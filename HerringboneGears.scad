...
module WadeL_double_helix(){
	...
	difference(){
		...
		gear (number_of_teeth=teeth,
			...
			rim_width = 5,
			hub_thickness = height/2, // hub_thickness = height/2*1.5,
			hub_diameter = 25,
			bore_diameter = 8,
			circles=circles,
			twist = twist/teeth,
		...
		union () {
			...
			translate ([0,0,6]) rotate ([0,0,30]) hexagon(height=12.5,depth=12); // was height=13
		}
...
module WadesL(){
	...
	difference(){
		...
		union () {
			translate([0,0,10])rotate([180,0,0])cylinder(r=4.1,h=20,$fn=circle_facets*10);
			translate ([0,0,11]) rotate ([0,0,30]) hexagon(height=12.5,depth=10);// was height=14
		}
	}
}
...
module WadesS(){
	...
	difference(){
		gear (number_of_teeth=11,
			...
			involute_facets = involute_facets,
			circle_facets = circle_facets);
		translate([0,-5,16])cube([5.5,2.4,9],center = true);
		translate([0,0,14])rotate([0,90,-90])cylinder(r=1.6,h=20,$fn=circle_facets*4);
	}
}
...
//		translate([0,-5,12])cube([5.5,2.4,9],center = true);
		translate([0,-5,12])cube([5.7,2.7,9],center = true);
		translate([0,0,11])rotate([0,90,-90])cylinder(r=1.6,h=20,$fn=circle_facets*4);
	}
}
